...
class TestSafety:
    ...
    def test_reorder_binds_before_use(self):
    ...
    def test_comparison_ordered_last(self):
        safe, rendered, _ = self.check("p(X) :- q(X), X < Y, r(Y).")
        assert safe
        assert rendered == "p(X) :- q(X); r(Y); X<Y."

    def test_equality_no_flip_when_left_binds(self):
    ...
    def test_equality_flip_when_right_binds(self):
    ...
    def test_unsafe_unbound_head_variable(self):
        safe, _, unsafe = self.check("p(X) :- q(Y).")
        assert not safe
        assert unsafe == ["X"]

    def test_unsafe_negative_literal_does_not_bind(self):
        safe, _, unsafe = self.check("p(X) :- not q(X).")
        assert not safe
        assert unsafe == ["X"]

    def test_unsafe_comparison_does_not_bind(self):
        # X < Y cannot bind X; only Y is provided by q(Y).
        safe, _, unsafe = self.check("p(X) :- X < Y, q(Y).")
        assert not safe
        assert unsafe == ["X"]

    def test_unsafe_reports_all_unbound(self):
    ...
    def test_linear_term_binds(self):
        # 1*X+1 is the canonical linear form; matching it binds X.
        safe, _, unsafe = self.check("p(X) :- q(1*X+1).")
        assert safe
        assert unsafe == []

    def test_nonlinear_arithmetic_does_not_bind(self):
        # X*X is not linear, so X is not provided by the match.
        safe, _, unsafe = self.check("p(X) :- q(X*X).")
        assert not safe
        assert unsafe == ["X"]

    def test_unary_minus_preserves_binding(self):
        safe, _, unsafe = self.check("p(X) :- q(-X).")
        assert safe

    def test_absolute_value_does_not_bind(self):
        safe, _, unsafe = self.check("p(X) :- q(|X|).")
        assert not safe
        assert unsafe == ["X"]

    def test_function_argument_binds(self):
        safe, _, _ = self.check("p(X) :- q(f(X)).")
        assert safe

    # --- aggregates --------------------------------------------------------

    def test_body_aggregate_left_guard_binds(self):
        safe, _, unsafe = self.check("p(N) :- N = #count{ X : q(X) }.")
        assert safe
        assert unsafe == []

    def test_body_aggregate_non_equal_guard_does_not_bind(self):
        # N < #count{...} cannot bind N.
        safe, _, unsafe = self.check("p(N) :- N < #count{ X : q(X) }.")
        assert not safe
        assert unsafe == ["N"]

    def test_local_unsafe_in_aggregate_element(self):
    ...
    def test_local_unsafe_in_aggregate_element(self):
        # Y is is both in the condition and on the guard.
    ...
    def test_body_conditional_literal_safe(self):
        safe, _, unsafe = self.check("p :- q(X) : r(X).")
        assert safe
        assert unsafe == []

    def test_head_disjunction_safe(self):
        safe, _, unsafe = self.check("a(X) ; b(X) :- c(X).")
        assert safe
        assert unsafe == []

    def test_constraint_no_variables(self):
    ...
    def test_show_term_must_be_bound(self):
        safe, _, _ = self.check("#show p(X) : q(X).")
        assert safe

    def test_show_term_unbound_is_unsafe(self):
        safe, _, unsafe = self.check("#show p(X) : q(Y).")
        assert not safe
        assert unsafe == ["X"]

    def test_external_body_checked(self):
        assert self.check("#external p(X) : q(X).")[0]
        safe, _, unsafe = self.check("#external p(X) : q(Y).")
        assert not safe
        assert unsafe == ["X"]

    def test_program_statement_trivially_safe(self):
    ...
    def test_multi_guard_comparison_raises(self):
        with pytest.raises(SafetyError):
            self.check("p(X) :- X = Y = Z.")

    def test_set_aggregate_raises(self):
        with pytest.raises(SafetyError):
...
class TestAnalyzeHelpers:
    ...
    def test_check_linear_recognises_canonical_form(self):
        term = ast.parse_term(self._lib, "1*X+1")
        assert check_linear(term) == "X"

    def test_check_linear_rejects_nonlinear(self):
    ...
    def test_select_variables_global_excludes_element_locals(self):
    ...
    def test_select_variables_global_includes_guards(self):
